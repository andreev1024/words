...
window.onload = function() {
    ...
    function learnWords(prevWord)
    {
        ...
        if (prevWord !== undefined) {
            const allWordsRepeatedTwice = allWords.concat(allWords);
            const index = allWordsRepeatedTwice.findIndex((word) => word.en === prevWord);
            if (index === -1) {
                throw new Error('cant find a proper word');
            }
            nextWord = allWordsRepeatedTwice[index + 1];
        }
